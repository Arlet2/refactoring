...
export default function UpdateTransport({
  ...
  const { transportNumber, packsCapacity, maxWeight } = updateTransportData;
  ...
  const updateTransport = () => {
    setErrorMsg("");
    transport.packsCapacity = Number(transport.packsCapacity);
    transport.maxWeight = Number(transport.maxWeight);

    if (transport.transportNumber == "") {
      setErrorMsg("Номер транспорта не может быть пустым!");
      return;
    }

    if (!Number.isInteger(transport.packsCapacity)) {
      setErrorMsg("Количество пачек должно быть целым числом!");
      return;
    }

    if (transport.packsCapacity < 0) {
      setErrorMsg("Количество пачек не может быть отрицательным!");
      return;
    }

    if (!Number.isFinite(transport.maxWeight)) {
      setErrorMsg("Максимальный вес должен быть числом!");
      return;
    }

    if (transport.maxWeight < 0) {
      setErrorMsg("Максимальный вес не может быть отрицательным!");
      return;
    }
    ...
    fetch(process.env.REACT_APP_API_PATH + "/transports", {
      ...
      .then((result) => {
        ...
        updateModalSetting();
        toast.info(
          "Транспорт " + transport.transportNumber + " успешно обновлён!",
          {
                        ...
                        <div className="grid gap-4 mb-4 sm:grid-cols-2">
                          <div>
                            ...
                            <input
                              ...
                              placeholder="Ш201АИ55"
                            />
                          </div>
                          <div>
                            <label
                              htmlFor="packs_capacity"
                              className="block mb-2 text-sm font-medium text-gray-900 dark:text-white"
                            >
                              Вместимость (пачки)
                            </label>

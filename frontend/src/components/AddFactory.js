...
export default function AddFactory({
  ...
  const handleInputChange = (key, value) => {
    setFactory({ ...factory, [key]: value });
  };

  const addFactory = () => {
    setErrorMsg("");
    if (factory.address == "") {
      setErrorMsg("Адрес не может быть пустым!");
      return;
    }
    ...
    fetch(process.env.REACT_APP_API_PATH + "/factories", {
      ...
      .then((result) => {
        if (result.status == 208) {
          setErrorMsg("Данная фабрика уже существует");
          return undefined;
        }
      ...
      .then((result) => {
        ...
        addFactoryModalSetting();
        toast.success(
          "Фабрика по адресу" + factory.address + " успешно создана!",
          {
